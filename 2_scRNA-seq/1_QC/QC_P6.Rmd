...
abline(v = 2200, col = "red")
```

```{r FilterTotalCounts}
filter_by_total_counts <- (umi2$total > 2200)
...
filter_by_expr_features <- (umi2$detected > 1100)
...
```{r FilterParameters}
param <- list()
param$total_counts <- 2200
param$expr_features <- 1100
param$MT <- 10
write.table(param, "path/to/001_filter_param_P6.txt", quote = F)
```
