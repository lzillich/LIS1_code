...
```{r FilterTotalCounts}
filter_by_total_counts <- (umi2$total > 3500)
...
filter_by_expr_features <- (umi2$detected > 700)
...
```{r FilterParameters}
param <- list()
param$total_counts <- 3500
param$expr_features <- 700
param$MT <- 10
write.table(param, "path/to/001_filter_param_P5.txt", quote = F)
```
