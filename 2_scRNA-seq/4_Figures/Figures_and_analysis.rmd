...
author: "Lea Zillich, Anne Hoffrichter, Eric Zillich"
date: "2022/02/17"
output:
  html_document:
    df_print: paged
...
ggsave("/path/to/figures/Fig2b_DimPlot.png", p1, width= 7, height = 5, dpi=600)
...
ggsave("/path/to/figures/SupFig2A_DimPlot_by_replicate.pdf", p2, width= 9, height = 6, dpi=600)
...
```{r celltypes stacked}
samples <- c(rep("Ctrl" , 10) , rep("Mild" , 10), rep("Moderate" , 10), rep("Severe" , 10))
...
ggsave(plot = per,filename ="path/to/figures/Fig2C_celltype_percentage.pdf", width = 3, height=5)
```



## Supplementary Figures 3C

```{r sut-RG fig 3c 3d, warning=FALSE, message=FALSE, results='hide'}
cell_perc$percent <- c(cell_perc$value[cell_perc$samples=="Ctrl"]/sum(cell_perc$value[cell_perc$samples=="Ctrl"]),
                       cell_perc$value[cell_perc$samples=="Mild"]/sum(cell_perc$value[cell_perc$samples=="Mild"]),
                       cell_perc$value[cell_perc$samples=="Moderate"]/sum(cell_perc$value[cell_perc$samples=="Moderate"]),
                       cell_perc$value[cell_perc$samples=="Severe"]/sum(cell_perc$value[cell_perc$samples=="Severe"]))

cell_prog <- data.frame(condition=rep(c("Ctrl","Mild","Moderate","Severe"),2),
                        type=c(rep("Progenitor",4),rep("Differentiated Cells",4)),
                        perc=c(sum(cell_perc$percent[cell_perc$samples=="Ctrl"& (cell_perc$celltypes=="RG"|cell_perc$celltypes=="t-RG"|cell_perc$celltypes=="NE")]),sum(cell_perc$percent[cell_perc$samples=="Mild"& (cell_perc$celltypes=="RG"|cell_perc$celltypes=="t-RG")]), sum(cell_perc$percent[cell_perc$samples=="Moderate"&  (cell_perc$celltypes=="RG" |cell_perc$celltypes=="t-RG"|cell_perc$celltypes=="NE")]), sum(cell_perc$percent[cell_perc$samples =="Severe"& (cell_perc$celltypes=="RG"|cell_perc$celltypes=="t-RG"|cell_perc$celltypes=="NE")]), sum(cell_perc$percent[cell_perc$samples=="Ctrl"& (cell_perc$celltypes =="dFB-N" |cell_perc$celltypes=="vFB-N"|cell_perc$celltypes=="MB-N"|cell_perc$celltypes=="IN"|cell_perc$celltypes=="G")]),sum(cell_perc$percent[cell_perc$samples=="Mild"& (cell_perc$celltypes=="dFB-N" |cell_perc$celltypes=="vFB-N"|cell_perc$celltypes=="MB-N"|cell_perc$celltypes=="IN"|cell_perc$celltypes=="G")]), sum(cell_perc$percent[cell_perc$samples=="Moderate"& (cell_perc$celltypes=="dFB-N" |cell_perc$celltypes=="vFB-N"|cell_perc$celltypes=="MB-N"|cell_perc$celltypes=="IN"|cell_perc$celltypes=="G")]),sum(cell_perc$percent[cell_perc$samples=="Severe"& (cell_perc$celltypes=="dFB-N" |cell_perc$celltypes=="vFB-N"|cell_perc$celltypes=="MB-N"|cell_perc$celltypes=="IN"|cell_perc$celltypes=="G")])))
...
bar_prog <- ggplot(df_bar_prog, aes(x=celltypes, y=percent, fill=samples, color=samples)) + ylim(0,1) +
...
ggsave(bar_prog,filename ="/path/to/figures/SupFig3C_barplot_progenitor.pdf", width = 10, height=7)
...
ggsave("/path/to/figures/SupFig2B_DotPlot.png", p3, width= 10, height = 5, dpi=600)
```
...
write.csv(markers_cl, paste0("/path/to/DEresults/progenitor_",i,".csv"), row.names = T)

}

save(sc, file="/path/to/seurat_objects/LIS1_integrated_DE.rds")
```


# Heatmap of DE genes across clusters - Figure 2D
...
venn.diagram(x=list(mild_sig$X,moderate_sig$X,severe_sig$X), category.names = c("mild", "moderate", "severe"),  fill = c("#F7AB64","#C7361B","#880000"),fontfamily ="calibri",cat.fontfamily = "calibri", cex = 2, cat.cex = 1.3, "/path/to/figures/SupFig3D_venn.png")
...
## RG and t-RG clusters - neurogenesis-related terms - Figure 2D
...
# p-values 
wil_t$neuron.differentiation$p.value 
wil_t$generation.of.neurons$p.value 
wil_t$neuron.fate.commitment$p.value 
wil_t$regulation.of.neuron.differentiation$p.value 
...
ggsave("/path/to/Fig2D_VlnPlot.png", vln, width= 5, height = 5, dpi=600)
```
...
# Plot heatmap
png("/path/to/figures/Fig4L_Wnt_CyP.png", width = 300,height=700)
