...
```{r FilterTotalCounts}
filter_by_total_counts <- (umi2$total > 2500)
...
filter_by_expr_features <- (umi2$detected > 1300)
...
```{r FilterParameters}
param <- list()
param$total_counts <- 2500
param$expr_features <- 1300
param$MT <- 10
write.table(param, "path/to/001_filter_param_P4.txt", quote = F)
```
